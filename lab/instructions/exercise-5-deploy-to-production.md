...
1. In the terminal output from the `npx rayfin up` command you ran in Exercise 4, find the **hosting URL** printed by the CLI.

    The URL should look similar to `https://<random-prefix>.webapp.rayfin….com`.
...
1. Confirm that you can see the same profile and work orders you created in the previous exercise.

    This verifies that the hosted app is using the same database as the app you tested locally.
...
1. Confirm that the workspace contains a **Fabric data app** item and a **SQL Database** item.

    The `ServicePro` and `WorkOrder` tables are stored in the SQL Database item.
