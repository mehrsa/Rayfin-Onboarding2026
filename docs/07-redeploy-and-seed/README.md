# 7. Redeploy and seed test data

Your new comments feature works locally. Time to push it to production and put it under realistic load by seeding the deployed app with a large generated dataset. This sets us up for [Step 8](../08-fabric-insights/README.md), where we'll point Fabric data agents at the production data.
...
This re-runs the same flow you saw in Step 5, but the CLI **detects your existing deployment** and updates it in place rather than provisioning a new one. It also picks up the schema change you made in [Step 6](../06-add-feature/README.md) and applies it to the production database automatically.
...
> ⚠️ If `rayfin up` warns about destructive changes, review the listed operations carefully before re-running with `--force`. Adding a new entity or new optional fields should not trigger this warning, but dropping or renaming columns will.
...
## 3. Seed the production database with `/_admin/`
...
Now go back to the Service Pro view at `/` and to the manager view at `/manager/`. You should see a much richer dataset: dozens of service pros across different skill sets, work orders in every status, addresses across multiple cities, etc.

> 🧠 **Why we're doing this now:** in [Step 8](../08-fabric-insights/README.md) we'll create a Fabric data agent over this app's database. A handful of seeded rows would make for boring queries — a few hundred makes the agent's natural-language answers actually interesting.

## 4. (Optional) Add some comments

Pick a few work orders and post a couple of comments on each. The data agent in Step 8 can answer questions about comments too, and a sparse comments thread limits what you can ask.
...
- `/_admin/` ran the seed and the deployed app now has dozens of service pros and work orders.
